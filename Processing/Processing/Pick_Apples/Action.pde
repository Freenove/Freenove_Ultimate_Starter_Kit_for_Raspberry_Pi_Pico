...
void apple() {
  apple_x = int(random(50,550));
  apple_y = int(random(50,400));
}

void bomb() {
  bomb_x = int(random(50,550));
  bomb_y = int(random(50,400));
  if(abs(bomb_x - apple_x) < 55 && abs(bomb_y - apple_y) < 55){
    bomb();
  }
}
...
void RunGame(){
    ...
    over();
    font();
    if(sqrt((apple_x - x)*(apple_x - x)+(apple_y - y)*(apple_y - y)) < 40) {
      apple();
      bomb();
      score += 20;
    }
    if(sqrt((bomb_x - x)*(bomb_x - x)+(bomb_y - y)*(bomb_y - y)) < 40) {
      apple();
      bomb();
      score -= 20;
    }
