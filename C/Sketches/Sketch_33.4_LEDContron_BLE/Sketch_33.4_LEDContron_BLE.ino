...
int gattWriteCallback(uint16_t value_handle, uint8_t *buffer, uint16_t size) {
  if (value_handle == CCCD_HANDLE) {                  // Processing CCCD writing
    ...
    String myString = String(characteristic_data);    // Convert to String
    myString.trim();                                  // Remove front and back blank spaces
    if(myString.equals("led_on") == true){            // led on
      led_status = true;
      digitalWrite(LED, led_status);
    }
    if(myString.equals("led_off") == true){           // led off
      led_status = false; 
      digitalWrite(LED, led_status);
    }
    if(myString.equals("led_toggle") == true){        // led toggle
