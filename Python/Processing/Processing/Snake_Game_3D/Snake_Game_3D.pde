...
void draw() {
  ...
  if (analogs != null)
  {
    lastAnalogs0 = analogs[0];
    lastAnalogs1 = analogs[1];
    if(lastAnalogs0 == 0 || lastAnalogs1 == 0)
    {
      lastAnalogs0 = beforeAnalogs0;
      lastAnalogs1 = beforeAnalogs1;
    }
    else
    {
      beforeAnalogs0 = lastAnalogs0;
      beforeAnalogs1 = lastAnalogs1;
    }
    if (lastAnalogs1 < 32767 - threshold)
    {    
      if (snake.direction != Direction.DOWN)
        snake.nextDirection = Direction.UP;
    } 
    else if (lastAnalogs1 > 32767 + threshold) {
      if (snake.direction != Direction.UP)
        snake.nextDirection = Direction.DOWN;
    }
    if (lastAnalogs0 < 32767 - threshold) {
      if (snake.direction != Direction.RIGHT)
        snake.nextDirection = Direction.LEFT;
    } 
    else if (lastAnalogs0 > 32767 + threshold) {
