...
void draw() {
  ...
  if (analogs != null)
  {
    lastLY = analogs[0] * height /65535; //10
    lastRY = analogs[1] * height /65535;
    if(abs(lastLY - beforeLY) > 4)
    {
      lPaddle.position.y = lastLY;
      beforeLY = lastLY;
    }
    else
    {
      lPaddle.position.y = beforeLY;
    }
    if(abs(lastRY - beforeRY) > 3)
    {
      rPaddle.position.y = lastRY;
      beforeRY = lastRY;
    }
    else
    {
      rPaddle.position.y = beforeRY;
    }
  }
